# Data Engineering Internship - Celebal Technologies

This repository contains all the assignments and projects completed during my Data Engineering internship at Celebal Technologies. It documents my learning journey and hands-on experience across Python, SQL, cloud platforms, and big data tools — from data manipulation basics to distributed processing with Spark and Delta Lake.

## Weekly Progress

| Week | Topic | Description |
|------|-------|-------------|
| Week 1 | Python & Pandas Basics | Core Python concepts and data manipulation using Pandas |
| Week 2 | SQL Basics | Fundamental SQL queries, joins, and filtering |
| Week 3 | Advanced SQL | Window functions, subqueries, and query optimization |
| Week 4 | Microsoft Azure | Introduction to Azure services for data engineering |
| Week 5 | Data Cleaning | Handling missing values, duplicates, and data quality checks |
| Week 6 | Spark Intro | PySpark fundamentals, transformations, and aggregations on the Sample Superstore dataset |
| Week 7 | Databricks & Delta Lake | Incremental data processing using Delta Lake MERGE operations |

## Repository Structure
Each week has a dedicated folder containing the tasks completed during that period. Depending on the assignment, folders may include:
- Jupyter Notebooks (`.ipynb`)
- SQL Scripts (`.sql`)
- Datasets (`.csv`, `.xlsx`)
- Screenshots documenting execution and output
- Reports and documentation (`README.md`, summaries)

```
Internship-Tasks/
│
├── Week1_Python_Pandas/
├── Week2_SQL_Basics/
├── Week3_Advanced_SQL/
├── Week4_Microsoft_Azure/
├── Week5_Data_Cleaning/
├── Week6_Spark_Intro/
├── Week7_Databricks_DeltaLake/
│
└── README.md
```

## Skills Demonstrated
- Data manipulation and analysis with Python and Pandas
- Relational database querying and optimization with SQL
- Cloud fundamentals on Microsoft Azure
- Data cleaning and quality assurance techniques
- Distributed data processing with Apache Spark (PySpark)
- Incremental data pipelines and upserts using Delta Lake

## Objective
The goal of this repository is to track my internship progress, demonstrate practical implementation of data engineering concepts, and maintain a structured, well-documented record of completed assignments and projects.

## Author
Lakshita
Data Engineering Intern — Celebal Technologies
